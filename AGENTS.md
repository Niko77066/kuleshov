...
全自动多源视频 agent 生产链路 **Kuleshov** 的第一形态：人 + Codex + ~10 份文件。
设计蓝图（v3.4）：https://Codex.ai/code/artifact/06e62b33-6e82-4ffc-b893-94a15ccd69a1

M0 的交付物不是视频，是：**验证过的品味资产 + 带人工分的片库（未来评委的校准语料）+ 稳定下来的 prompt 模式（未来编译器的输入）**。

## 目录

- `.Codex/skills/produce/` — 十阶段生产管线 SOP（入口：`/produce`），引擎知识包在其 `references/` 下，按镜头路由按需加载
