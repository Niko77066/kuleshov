...
3. **风格包**：列出 `styles/` 下可用包让用户选——当前 `pixel-chronicle`（像素科普·横屏）/ `case-file`（案卷调查·竖屏）/ `whiteboard-generalist`（公测兜底）；选定后执行该包的开拍提问模板。**选题超出前两个专用包预期、或低置信/混合输入 → 落 `whiteboard-generalist` 兜底**。`_` 前缀目录（`_template` / `_disabled`）不是风格包、不列入。`meme-ledger` 已完成一次试跑但未通过独立风格验收，继续暂禁用（在 `styles/_disabled/`）；下一条须通过 `/video-score`、结构复盘与资产授权清单后再开放。
