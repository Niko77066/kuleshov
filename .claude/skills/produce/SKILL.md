...
4. **风格包先行**：没选风格包不开拍。风格包在 `styles/` 下，含开拍提问模板的要执行它的提问。有 `contract.json` 的包，合同是机器硬门不是参考读物——生产期内合同文件只读，调整只能走带宽内 amendments。
5. **铁律见根目录 CLAUDE.md**：先写 IR 再花钱、留痕、音频先行、禁静默降级（含**表达降级**）。
6. **端到端授权要留痕**：用户授权代决时，把委托范围写进 `meta.approval_policy`（如 `{"mode":"delegated","delegate":"ep","ask_on":["taste_constitution"]}`）——EP 代决的门在 ledger 里记 `by: "ep(delegated)"`，**不许写成"用户已批准"**。此时评委门由 G2 harness 代行（③b 与 ⑨.3），不许用自评顶替。
...
### ③b hero-frames 品味门 ⏸（2026-07-21 起，无条件触发）
...
3. 评审：人在环时 ⏸ 用户看；**端到端授权时由 G2 评委盲评**（`tools/judge/`，证据包只含并排图与镜头意图，不含创作理由），未达 Golden 下限**禁止铺开全片 storyboard**，换视觉方案重来；
...
3. ⏸ **用户亲自看片**（M0 的评委团就是用户）。**端到端授权时由 G2 评委代行**：`tools/judge/build_evidence_pack.py` 出证据包（contact sheet + Golden 并排 + L0 + 实测指标，隔离创作上下文）→ `tools/judge/judge.py --node final` 盲评，报告记 `ledger.gates`；评委扣分必须引用镜头 ID/时间码，否则判无效。评委否决权按"先校准后放权"（`tools/judge/README.md` 校准协议）。
