...
5. **铁律见根目录 CLAUDE.md**：先写 IR 再花钱、留痕、音频先行、禁静默降级。
...
4. **时长承诺**：如 60s ± 5%。
5. **通路预算**：
   - **零成本通路**（默认）：HyperFrames + 图片动效 + TTS——先跑通全链路骨架；
   - **完整通路**：追加 Seedance 生成视频（走 new-api 网关，花真钱），只用来验证"运动注入"那一段增量。
...
### ⑥ anchors（按需）⏸
零成本通路：风格包的 CSS tokens / 版式模板即锚点，可跳过本阶段。
...
| Provider | M0 状态 | 擅长 | 关键约束 | 知识包 |
|---|---|---|---|---|
| HyperFrames | ✅ 可用 | 动态排版、数据卡、标题、字幕层 | 产出属"幻灯片语法"，不计运动占比 | `references/hyperframes.md` |
| 图片 + 动效 | ⚙️ 需接入 GPT-Image-2 | 风格化静帧、概念示意、"准运动" | 连续 ≤ 2 镜；降级运动 | `references/image-motion.md` |
| TTS 音频 | ⚙️ 需接入 | 一切旁白 | 旁白永远走 TTS 轨 | `references/tts-audio.md` |
| Seedance 2.0 | ⚙️ 网关已定待配置 | 真运动、氛围、hook/hero 镜头 | 单次 4–15s、720p；必须挂锚点 | `references/seedance.md` |
| 数字人 / 检索剪辑 | ⛔ M2/M3 再开 | — | — | — |

路由纪律：要真运动的镜头**禁止**路由到 HTML（task_fit 归零）；图片动效连续 ≤ 2 镜；每个路由决策把理由和备选项记入 `ledger.decisions`。
...
- `ledger.costs` 累计超 `meta.budget.cap` 即熔断停机；
